...
Agent が `<!--reply:JSON-->` 形式の文字列を返した場合、Router は JSON をパースして `ReplyPayload` として Platform に渡す。
これにより Agent がプラットフォーム固有の UI コンポーネント (セレクトメニュー等) を返せる。
