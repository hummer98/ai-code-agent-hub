...
**対象:** 実サーバーでのメッセージ → スレッド → 応答フロー
**外部依存:** Discord API、テスト用 Discord サーバー、テスト用 Bot、Portal HTTP サーバー
**実行:** `npm run test:e2e` (`vitest.e2e.config.ts` — timeout 90秒)

### 実装状況

| ファイル | 状態 | スキップ条件 |
|---------|------|------------|
| `tests/e2e/portal.e2e.test.ts` | 実装済み | `PORTAL_URL` 未設定かつ非 CI |
| `tests/e2e/discord.e2e.test.ts` | 実装済み | `DISCORD_TEST_BOT_TOKEN` 未設定 |
| `tests/e2e/slack.e2e.test.ts` | スタブ (`describe.skip`) | 常にスキップ |
...
```env
# Discord E2E
DISCORD_TEST_BOT_TOKEN=   # テスト用 Bot トークン
# Hub Bot ID: 1439672503810261083 (ハードコード)
# Test Channel ID: 1479946719075635274 (ハードコード)

# Portal E2E
PORTAL_URL=               # Portal URL (デフォルト: http://172.25.76.16:3000)
PORTAL_PASSWORD=           # Basic Auth パスワード (設定時は認証テストを実行)
```
...
`PORTAL_PASSWORD` の有無に応じて認証なし/認証ありのテストブロックが切り替わる。

テスト内容:
- `GET /api/repos` が JSON 配列を返す
- `GET /` が HTML (`AI Code Agent Hub` を含む) を返す
- `PORTAL_PASSWORD` 設定時: 認証なし→401、正しい認証→200、不正パスワード→401
